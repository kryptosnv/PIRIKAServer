...
/**
 * The class implements the Prol Graphic library and it can be used to show to draw simple graphic primitives and save images. It uses the JDK graphic possibilities at its work.
...
public final class ProlGraphicLibrary extends ProlAbstractLibrary implements WindowListener, ActionListener {
    ...
    /**
     * The class describes an registered UI action which can be called through a menu item
     */
    ...
    /**
     * Inside logger, the canonical class name is used as the logger identifier (ProlGraphicLibrary.class.getCanonicalName())
     */
    ...
    public ProlGraphicLibrary() {
        ...
        graphicFrame = new JFrame("A Prol Graphic Screen");
        ...
        final JMenuItem menuItemSaveImage = new JMenuItem("To disk");
        menuItemSaveImage.setActionCommand("DISK");
        menuItemSaveImage.addActionListener(this);
        final JMenuItem menuItemCopyToClipboard = new JMenuItem("To clipboard");
     ...
     * @param type the type of the image format (jpg, png or gif, remember that it relates to possibilities of the ImageIO API from JRE)
     ...
     * @param width the new width of the back buffer at pixels, must be more than zero
     * @param height the new height of the back buffer at pixels, must be more than zero
     */
     ...
     * @return a Term which represents the color from the argument, the RGB color as #RRGGBB
     */
    ...
    /**
     * Inside function to convert a string representation of a color (as an example 'red' or '#FF0000') into its Color object representation
     * @param text the text to be decoded, must not be null
     * @return the Color object or null if it is impossible to decode the text value
     */
    private static Color textToColor(String text) {
        boolean colorAsInteger = false;
        if (text.length() == 0) {
            return null;
        }
        try {
